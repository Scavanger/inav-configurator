...
const WEBVIEW = {
    ...
    advices: {
        installGoogleAndroidWebview: function(callback) {
            $(WEBVIEW.htmlElements.webview_step_msg).html(chrome.i18n.getMessage('cordovaWebviewInstall', {
                app: WEBVIEW.apps[WEBVIEW.appsId.googleWebview].name,
            }));
            $('#webview_step_btn1').text(chrome.i18n.getMessage('cordovaWebviewInstallBtn'))
                .attr('app_id', WEBVIEW.appsId.googleWebview);
            callback();
        },
        updateGoogleAndroidWebview: function(callback) {
            $(WEBVIEW.htmlElements.webview_step_msg).html(chrome.i18n.getMessage('cordovaWebviewUpdate', {
                app: WEBVIEW.apps[WEBVIEW.appsId.googleWebview].name,
            }));
            $(WEBVIEW.htmlElements.webview_step_btn1).text(chrome.i18n.getMessage('cordovaWebviewUpdateBtn'))
                .attr('app_id', WEBVIEW.appsId.googleWebview);
            callback();
        },
        ...
        uninstallGoogleAndroidWebview: function(callback) {
            $(WEBVIEW.htmlElements.webview_step_msg).html(chrome.i18n.getMessage('cordovaWebviewUninstall', {
                app: WEBVIEW.apps[WEBVIEW.appsId.googleWebview].name,
            }));
            $(WEBVIEW.htmlElements.webview_step_btn1).text(chrome.i18n.getMessage('cordovaWebviewUninstallBtn1'))
                .attr('app_id', WEBVIEW.appsId.googleWebview);
            $(WEBVIEW.htmlElements.webview_step_btn2).text(chrome.i18n.getMessage('cordovaWebviewUninstallBtn2'))
                .attr('app_id', WEBVIEW.appsId.googleWebview)
                .show();
            callback();
        },
        selectWebview: function(id, callback) {
            ...
            $(WEBVIEW.htmlElements.webview_step_msg).html(chrome.i18n.getMessage('cordovaWebviewEnable', {
                app: app,
            }));
            $(WEBVIEW.htmlElements.webview_step_btn1).hide();
            $(WEBVIEW.htmlElements.webview_step_btn2).text(chrome.i18n.getMessage('cordovaWebviewEnableBtn')).show();
            callback();
        },
    },
    ...
    tryToFindUsedApp: function(callback) {
        ...
        for (let i=0; i<appsId.length; i++) {
            ...
            if (self.matchingVersion === 1 && self.apps[id].used === 'could') {
                ...
                self.usedApp = id;
                $(`li[app_id='${id}']`).append(` (<span style="color: green">${chrome.i18n.getMessage('cordovaWebviewUsed')}</span>)`);
            }
